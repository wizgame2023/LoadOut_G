...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage15::CreateViewLight() {
	...
	void GameStage15::OnCreate() {
		...
		//最後にプレイしたステージを渡す
		App::GetApp()->GetScene<Scene>()->SetLastPlayStage(15);
	...
	void GameStage15::OnUpdate()
	{
	}

	//Player生成
	void GameStage15::CreatePlayer()
	{
	...
	//アイテム生成
	void GameStage15::CreateBattery()
	{
		vector<Vec3> posVec =
		{
	...
	//敵がどれくらい追いかけてくるか確認する
	int GameStage15::GameEnemyState()
	{
	...
	void GameStage15::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage15::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
