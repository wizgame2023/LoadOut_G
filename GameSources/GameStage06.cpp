...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage06::CreateViewLight() {
	...
	void GameStage06::OnCreate() {
		...
		//最後にプレイしたステージを渡す
		App::GetApp()->GetScene<Scene>()->SetLastPlayStage(6);
		...
		//ステージマネージャー生成
		auto stageManager = AddGameObject<StageManager>(3,15.0f,12.0f);
		...
		stageManager->ClearMode(1);//一定数敵を倒すと鍵が手に入るモード
	...
	void GameStage06::OnUpdate()
	{
	}

	//Player生成
	void GameStage06::CreatePlayer()
	{
	...
	//Enemy生成
	void GameStage06::CreateEnemy()
	{
		vector<shared_ptr<Enemy>>enemyVec;
		vector<Vec3> posVec =
		{
			Vec3(40.0f, 0.0f, 60.0f),
			Vec3(-40.0f, 0.0f, 60.0f),
	...
	//アイテム生成
	void GameStage06::CreateBattery()
	{
		vector<Vec3> posVec =
		{
	...
	//ランダムアイテム生成
	void GameStage06::CreateRamdomItem()
	{
		vector<Vec3> posVec =
		{
		   Vec3(70.0f, 2.5f, -30.0f),//1
		   Vec3(-30.0f, 2.5f, 60.0f),//2
	...
	//敵がどれくらい追いかけてくるか確認する
	int GameStage06::GameEnemyState()
	{
	...
	void GameStage06::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage06::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
