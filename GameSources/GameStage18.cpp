...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage18::CreateViewLight() {
	...
	void GameStage18::OnCreate() {
		...
		//最後にプレイしたステージを渡す
		App::GetApp()->GetScene<Scene>()->SetLastPlayStage(18);
	...
	void GameStage18::OnUpdate()
	{
	}

	//Player生成
	void GameStage18::CreatePlayer()
	{
	...
	//アイテム生成
	void GameStage18::CreateBattery()
	{
		vector<Vec3> posVec =
		{
	...
	//敵がどれくらい追いかけてくるか確認する
	int GameStage18::GameEnemyState()
	{
	...
	void GameStage18::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage18::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
