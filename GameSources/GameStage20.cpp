...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage20::CreateViewLight() {
	...
	void GameStage20::OnCreate() {
		...
		//最後にプレイしたステージを渡す
		App::GetApp()->GetScene<Scene>()->SetLastPlayStage(20);
	...
	void GameStage20::OnUpdate()
	{
	}

	//Player生成
	void GameStage20::CreatePlayer()
	{
	...
	//アイテム生成
	void GameStage20::CreateBattery()
	{
		vector<Vec3> posVec =
		{
		Vec3(-5.0f, 2.5f, -24.0f),//1
		Vec3(-25.0f, 2.5f, 45.0f),//2
		Vec3(-64.0f, 2.5f, 14.0f),//3
	...
	//敵がどれくらい追いかけてくるか確認する
	int GameStage20::GameEnemyState()
	{
	...
	void GameStage20::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage20::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
