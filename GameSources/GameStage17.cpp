...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage17::CreateViewLight() {
	...
	void GameStage17::OnCreate() {
		...
		//最後にプレイしたステージを渡す
		App::GetApp()->GetScene<Scene>()->SetLastPlayStage(17);
	...
	void GameStage17::OnUpdate()
	{
	}

	//Player生成
	void GameStage17::CreatePlayer()
	{
	...
	//アイテム生成
	void GameStage17::CreateBattery()
	{
		vector<Vec3> posVec =
		{
	...
	//敵がどれくらい追いかけてくるか確認する
	int GameStage17::GameEnemyState()
	{
	...
	void GameStage17::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage17::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
