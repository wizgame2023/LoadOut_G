...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage03::CreateViewLight() {
	...
	void GameStage03::OnCreate() {
		...
		//最後にプレイしたステージを渡す
		App::GetApp()->GetScene<Scene>()->SetLastPlayStage(3);

		//マップマネージャーの生成
		auto mapManager = AddGameObject<MapManager>(L"Stage03/");
		...
		//ステージマネージャー生成
		auto stageManager = AddGameObject<StageManager>(2, 15.0f,10.0f);
		...
		stageManager->ClearMode(2,2);//特定の敵を倒すと鍵が手に入るモード
	...
	void GameStage03::OnUpdate()
	{
	}

	//Player生成
	void GameStage03::CreatePlayer()
	{
	...
	//Enemy生成
	void GameStage03::CreateEnemy()
	{
		vector<shared_ptr<Enemy>>enemyVec;
		vector<Vec3> posVec =
		{
			Vec3(-25.0f, 0.0f, -15.0f),
			Vec3(15.0f, 0.0f, 35.0f)
		};
	...
	//アイテム生成
	void GameStage03::CreateBattery()
	{
		vector<Vec3> posVec =
		{
	...
	//敵がどれくらい追いかけてくるか確認する
	int GameStage03::GameEnemyState()
	{
	...
	void GameStage03::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage03::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
