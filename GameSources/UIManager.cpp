...
namespace basecross {
	...
	void UIManager::OnCreate()
	{
		...
		if (StageMode == 1)
		{
			GetStage()->AddGameObject<Sprite>(L"EnemyUpText2", Vec2(180.0f, 100.0f), Vec3(-540.0f, 100.0f, 0.0f));//敵を倒した数(テキスト)
		}
		//複数敵を倒して鍵を入手するモード処理
		if (StageMode == 2)
		{
			...
			m_spriteNum = stage->AddGameObject<NuberManager>(enemyUpCount, Vec2(20.0f, 20.0f), Vec3(-640.0f + 130.0f, 400 - 300.0f, 0.0f), false);//敵を打ち上げる合計数
