...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage07::CreateViewLight() {
	...
	void GameStage07::OnCreate() {
		...
		//最後にプレイしたステージを渡す
		App::GetApp()->GetScene<Scene>()->SetLastPlayStage(7);

		//マップマネージャーの生成
		auto mapManager = AddGameObject<MapManager>(L"Stage07/");
		...
		//ステージマネージャー生成
		auto stageManager = AddGameObject<StageManager>(4,15.0f,8.0f);
	...
	void GameStage07::OnUpdate()
	{
	}

	//Player生成
	void GameStage07::CreatePlayer()
	{
	...
	//アイテム生成
	void GameStage07::CreateBattery()
	{
		vector<Vec3> posVec =
		{
	...
	void GameStage07::CreateRamdomItem()
	{
		vector<Vec3> posVec =
		{
		   Vec3(50.0f, 2.5f, 60.0f),//1
		   Vec3(10.0f, 2.5f, 10.0f),//2
	...
	void GameStage07::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage07::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
