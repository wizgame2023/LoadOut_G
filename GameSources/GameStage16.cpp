...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage16::CreateViewLight() {
	...
	void GameStage16::OnCreate() {
		...
		//最後にプレイしたステージを渡す
		App::GetApp()->GetScene<Scene>()->SetLastPlayStage(16);
	...
	void GameStage16::OnUpdate()
	{
	}

	//Player生成
	void GameStage16::CreatePlayer()
	{
	...
	//アイテム生成
	void GameStage16::CreateBattery()
	{
		vector<Vec3> posVec =
		{
	...
	//敵がどれくらい追いかけてくるか確認する
	int GameStage16::GameEnemyState()
	{
	...
	void GameStage16::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage16::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
