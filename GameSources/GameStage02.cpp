...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage02::CreateViewLight() {
	...
	void GameStage02::OnCreate() {
		...
		//最後にプレイしたステージを渡す
		App::GetApp()->GetScene<Scene>()->SetLastPlayStage(2);

		//マップマネージャーの生成
		auto mapManager = AddGameObject<MapManager>(L"Stage02/");
		...
		stageManager->ClearMode(2,1);//特定の敵を倒すと鍵が手に入るモード
	...
	void GameStage02::OnUpdate()
	{
	}

	//Player生成
	void GameStage02::CreatePlayer()
	{
	...
	//アイテム生成
	void GameStage02::CreateBattery()
	{
		vector<Vec3> posVec =
		{
		   //Vec3(25.0f, 2.5f, -35.0f),//1
		   Vec3(25.0f, 2.5f, 5.0f),//2
		   Vec3(-5.0f, 2.5f, 25.0f), //3
		   Vec3(-35.0f, 2.5f, -5.0f),//4
		   //Vec3(-15.0f, 2.5f, -45.0f)//5
		};
		auto test = posVec.size();
	...
	//敵がどれくらい追いかけてくるか確認する
	int GameStage02::GameEnemyState()
	{
	...
	void GameStage02::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage02::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
