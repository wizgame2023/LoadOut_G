...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage19::CreateViewLight() {
	...
	void GameStage19::OnCreate() {
		...
		//最後にプレイしたステージを渡す
		App::GetApp()->GetScene<Scene>()->SetLastPlayStage(19);
	...
	void GameStage19::OnUpdate()
	{
	}

	//Player生成
	void GameStage19::CreatePlayer()
	{
	...
	//アイテム生成
	void GameStage19::CreateBattery()
	{
		vector<Vec3> posVec =
		{
	...
	//敵がどれくらい追いかけてくるか確認する
	int GameStage19::GameEnemyState()
	{
	...
	void GameStage19::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage19::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
