...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage05::CreateViewLight() {
	...
	void GameStage05::OnCreate() {
		...
		stageManager->ClearMode(2,3);//特定の敵を倒すと鍵が手に入るモード
	...
	void GameStage05::OnUpdate()
	{
	}

	//Player生成
	void GameStage05::CreatePlayer()
	{
	...
	//Enemy生成
	void GameStage05::CreateEnemy()
	{
		vector<shared_ptr<Enemy>>enemyVec;
		vector<Vec3> posVec =
		{
			Vec3(-35.0f, 0.0f, 25.0f),
			Vec3(35.0f, 0.0f, 25.0f),
	...
	//アイテム生成
	void GameStage05::CreateBattery()
	{
		vector<Vec3> posVec =
		{
	...
	//敵がどれくらい追いかけてくるか確認する
	int GameStage05::GameEnemyState()
	{
	...
	void GameStage05::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage05::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
