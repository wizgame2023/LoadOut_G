...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage13::CreateViewLight() {
	...
	void GameStage13::OnCreate() {
		...
		//最後にプレイしたステージを渡す
		App::GetApp()->GetScene<Scene>()->SetLastPlayStage(13);
	...
	void GameStage13::OnUpdate()
	{
	}

	//Player生成
	void GameStage13::CreatePlayer()
	{
	...
	//アイテム生成
	void GameStage13::CreateBattery()
	{
		vector<Vec3> posVec =
		{
	...
	//敵がどれくらい追いかけてくるか確認する
	int GameStage13::GameEnemyState()
	{
	...
	void GameStage13::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage13::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
