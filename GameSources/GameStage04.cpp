...
namespace basecross {
	...
	//	ゲームステージクラス実体
	//--------------------------------------------------------------------------------------
	void GameStage04::CreateViewLight() {
	...
	void GameStage04::OnCreate() {
		...
		//最後にプレイしたステージを渡す
		App::GetApp()->GetScene<Scene>()->SetLastPlayStage(4);

		//マップマネージャーの生成
		auto mapManager = AddGameObject<MapManager>(L"Stage04/");
	...
	void GameStage04::OnUpdate()
	{
	}

	//Player生成
	void GameStage04::CreatePlayer()
	{
	...
	//アイテム生成
	void GameStage04::CreateBattery()
	{
		vector<Vec3> posVec =
		{
	...
	//敵がどれくらい追いかけてくるか確認する
	int GameStage04::GameEnemyState()
	{
	...
	void GameStage04::OutWallCreate(int selLength)//←個々の引数はこのマップの直径はブロック何個ぶんかを聞いています
	{
	...
	void GameStage04::SetEnemy(int num)
	{
		EnemyNow -= num;
	}
}
